...
model = UTitanIVAGModel(N,K,num_layers,N_updates_W=N_updates_W,N_updates_C=N_updates_C,epsilon=epsilon,archi=archi,custom=False)
...
scheduler_args_StepLR = {'gamma':0.8,'step_size':stepsize}
scheduler = torch.scheduler.ReduceLROnPlateau
...
trainer = UTitanTrainer(model,model_path,training_set,eval_set,training_mode=training_mode,optimizer=optimizer,optimizer_args=optimizer_args_SGD,scheduler=scheduler,scheduler_args=scheduler_args_StepLR,gradient_processing=gradient_processing,loss_train=loss_train,num_epochs=20,batch_size=100)
test.train()
